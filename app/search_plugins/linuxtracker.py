    ...
    class LinuxSearchParser(HTMLParser):
        ...
        def handle_data(self, data):
            if self.wait_for_data is True:
                ...
                elif self.strong_count == 2 and self.curr:
                    # Get pub_date (format: MM/DD/YYYY) and convert to Unix timestamp
                    try:
                        dt = datetime.datetime.strptime(data.strip(), "%m/%d/%Y")
                        self.curr['pub_date'] = int(dt.timestamp())
                    except ValueError:
                        pass
