// Stellar custom Controls style — ComboBox.
//
// Based on Qt's Material ComboBox (Copyright (C) 2017 The Qt Company Ltd.,
// SPDX-License-Identifier: LicenseRef-Qt-Commercial OR LGPL-3.0-only OR
// GPL-2.0-only OR GPL-3.0-only). Stellar changes, both for the software scene
// graph (our fallback when hardware OpenGL is unavailable — VirtualBox/SVGA3D,
// etc.): the popup renders in-overlay (popupType: T.Popup.Item) instead of a
// separate top-level window, and its background is a plain bordered rectangle
// with no shader elevation layer (a ShaderEffect the software backend can't run,
// which otherwise leaves the dropdown transparent). See CLAUDE.md
...
T.ComboBox {
    ...
    popup: T.Popup {
        // In-overlay so the dropdown is solid on every backend (see header).
        ...
        // Plain bordered rectangle, no shader elevation layer (see header).
