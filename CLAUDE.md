...
**Grabber Subsystem:**
- `app/src/grabber/GrabberCrawler.{h,cpp}` — Async BFS crawler via `QNetworkAccessManager`; crawls to depth, extracts links matching include/exclude wildcards, probes sizes via HEAD (4 concurrent). Emits `resultsFound`/`resultMetadataUpdated`/`progressChanged`/`finished`/`failed`. **SSRF gate (CWE-918):** every outbound target — start URL, page candidates, metadata HEAD probes, and each redirect hop — goes through `guardHostThen()` (literal IP synchronous, hostname via async DNS, result cached, fail-closed on empty/unresolved). Private/loopback/link-local/CGNAT/ULA hosts (incl. `169.254.169.254` metadata) are blocked; `guardRedirects()` aborts cross-host redirects to unsafe targets. `m_pendingGuards` keeps `finishIfDone()` from completing while a DNS lookup is outstanding. Page bodies capped at 16 MiB (`kMaxPageBytes`, early-abort on `downloadProgress`) against memory-DoS. Pattern regexes are precompiled in `start()` on the main thread (worker threads only read the cache — never mutate it).
