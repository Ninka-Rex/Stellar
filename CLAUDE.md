...
- **Network binding** (`torrentBindInterface`): resolved to actual IPs via `QNetworkInterface`; supports interface names and display names. Passed to libtorrent's `listen_interfaces`.
- **Torrent settings** all share one `torrentSettingsChanged` signal; `AppController` calls `torrentSession->applySettings()` on every change. Settings: `torrentEnableDht/Lsd/Upnp/NatPmp`, `torrentListenPort` (default 6881), `torrentConnectionsLimit`, `torrentDownloadLimitKBps`/`torrentUploadLimitKBps`, share-limit defaults, `torrentCustomUserAgent`, `torrentBindInterface`, `torrentEncryptionMode`, `torrentAutoBanAbusivePeers`, `torrentAutoBanMediaPlayerPeers`.
