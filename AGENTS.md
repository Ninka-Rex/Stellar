...
- `app/src/AppSettings.{h,cpp}` - Persistent QSettings storage, property binding with Q_PROPERTY
...
- `app/src/DownloadDatabase.{h,cpp}` - SQLite persistence for download history
...
- `app/qml/SettingsDialog.qml` - Tabbed settings with Apply/Cancel, dirty tracking. Tab indices: 0=Connection, 1=Categories, 2=Downloads, 3=Browser, 4=Speed Limiter, 5=Notifications, 6=General, 7=Media, 8=About
...
- `app/qml/FilePropertiesDialog.qml` - Properties window for any download (HTTP/FTP/torrent). For torrents: shows info hash, transfer stats, and per-torrent speed limits. Includes right-click rename context menu for files with `TapHandler` pattern
...
- Other dialogs: `AddUrlDialog.qml`, `FilePropertiesDialog.qml`, `BatchDownloadDialog.qml`, etc.
...
- **Torrent settings** all share one `torrentSettingsChanged` signal; `AppController` connects to it and calls `torrentSession->applySettings()` on every change. Settings: `torrentEnableDht/Lsd/Upnp/NatPmp`, `torrentListenPort` (default 6881), `torrentConnectionsLimit`, `torrentDownloadLimitKBps`/`torrentUploadLimitKBps`, share-limit defaults, `torrentCustomUserAgent`, `torrentBindInterface`.
