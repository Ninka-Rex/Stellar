...
function shouldPreIntercept(anchor, href) {
    ...
    // Click-time interception is explicit-intent only to avoid hijacking media
    // playback links before the page player can open them.
        ...
        try {
            ...
            if (href && this.hasAttribute("download")) {
                ...
                const timer = setTimeout(() => {
                    ...
                    // Do not auto-fallback to browser tray on timeout; keep this
                    // flow owned by Stellar interception.
                }, 5000);
                pendingProgrammatic.set(reqId, { anchor: this, timer });
                window.postMessage({
                    __stellarType: "programmaticDownloadIntent",
                    reqId,
                    href,
                    filename
                }, "*");
...
async function tryInterceptUrl(url, filename, explicitIntent) {
    ...
    try {
        ...
        let resp = await browser.runtime.sendMessage(payload);
        if (resp?.ok) return true;
        // Retry once to tolerate background cold-start wake latency.
        await new Promise((r) => setTimeout(r, 120));
        resp = await browser.runtime.sendMessage(payload);
        return !!resp?.ok;
    } catch {
        return false;
    }
...
document.addEventListener("click", async (event) => {
    ...
    if (!shouldPreIntercept(anchor, href)) return;
    const explicitIntent = anchor.hasAttribute("download");
